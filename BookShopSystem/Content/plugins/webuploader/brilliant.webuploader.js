    ...
    */
    ...
    BrilliantWebUploader.prototype = {
        ...
        _render: function () {
            ...
            function addFile(file, set) {
                ...
                var $li = $('<li id="' + file.id + '">' +
                    ...
                    "<p class=\"progress_check\" data-checkedcomple=\"" + flag + ">正在验证文件：<span>0</span>%</p>" +
                    liext +
                    '</li>'),
                    ...
                    switch (index) {
                        case 0:
                            if (set) {
                                ...
                                //移除
                                $("#" + file.id).remove();
                                $lic.off().find('.file-panel').off().end().remove();
                            }
                            else {
        ...
        setData: function (data) {
            var that = this;
            var json = {};
            json.id = "WU_CUSTOM_FILE_" + data.id;
            json.name = data.name;
            json.path = data.path;
            json.size = data.size;
            json.extension = data.ext;
            json.md5 = data.md5;
            json.type = data.type;
            json.fullPath = data.fullPath;
            json.lastModifiedDate = data.lastModifiedDate;
            json.getStatus = function () { };
            that._addFile(json, true);
            setTimeout(e => {
                _appendULTarget(json, "", "id");
            }, 1000)
